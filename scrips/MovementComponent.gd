...
func apply_gravityx(delta: float) -> void:
	if body == null:
		return
	body.velocity -= body.get_real_velocity() * delta * stop
	
func dash(dir_x: float) -> void:
	if body == null:
		return
	body.velocity.x = dir_x * dash_force
